...
- Confirm the cover animates into the larger artwork tile without a blank flash, then retraces the same path without clipping, jumping, or exposing a hard window edge when information mode closes. Its image, border, and highlight must follow the same rounded corners with no square gaps throughout both transitions.
...
- At 100% and 150% display scaling, confirm the desktop remains visible through the attached glass surface and that every tab selection, scrolling, the lifted artwork, window controls, and playback pill remain aligned.
