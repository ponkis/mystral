...
- Confirm the existing playback-controls pill stays in its upper-right position while the elapsed time, seek slider, and remaining time move together into the open space beneath the information content. The timeline must appear only once, with no enclosing fill or top divider; the compact cover, title, and artist must not remain duplicated.
...
- Confirm album tracks use compact number/title/artist/duration rows separated by the crisp one-pixel `cd_thing.png` fade beginning after the number gutter and continuing through the title/duration area; it must fade at both ends without truncating midway, and there must be no blurred band or row fill. Album and Track list headings remain white.
