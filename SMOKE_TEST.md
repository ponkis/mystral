...
- In the attached pane, verify lyrics begin near the top with the same compact inset as regular lyrics. The active synchronized line must use the stretched `cd_thing.png` fill plus crisp top and bottom edges, while other lines remain unframed; at the last line, the preceding line fades away and the song-details footer remains fully readable beneath it. Also verify automatic following, manual wheel scrolling, seekable line clicks, wait dots during long gaps, and readable plain lyrics. Switch tracks while it is open and confirm the loading state replaces the old lines; if the new track has no lyrics, the pane retracts cleanly.
