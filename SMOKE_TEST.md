...
- In regular lyrics, the information-side pane, and fullscreen lyrics, confirm the active synchronized line uses one stretched `cd_thing.png` fill with crisp top and bottom edges. Check a wrapped active line, seek between lines, switch tracks, and reopen fullscreen lyrics at the same playback position; the glow must follow the active row every time, while inactive and plain lines stay unframed.
