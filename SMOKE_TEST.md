...
- Confirm the seek and volume tooltips appear centered above the slider handle (pill) and follow it while dragging rather than sitting over the middle of the bar. Enter and leave information mode, then hover the compact seek bar without dragging; the tooltip must still open above the handle instead of retaining the information footer's wider position.
...
- On each of the Track, Artist, and Album tabs, click Lyrics and confirm a narrow glass pane slides out from beneath the information sheet's right side. The selected tab and its content must remain unchanged, with no switch to the regular lyrics player or compact-player flash.
- Toggle the attached lyrics pane repeatedly and confirm its full slide under the sheet and any monitor-edge repositioning reverse smoothly without a resize snap, clipped shadow, stale frame, doubled border, or jump in the main information sheet. Its reveal edge must stay fixed at the sheet seam, with no wallpaper gap, rounded inner edge, or shadow halo. The moving pane must never paint above the sheet on a normally fitting display. Press Escape once to retract the pane and again to close information.
- In the attached pane, verify lyrics begin near the top with the same compact inset as regular lyrics. The active synchronized line must retain the shared stretched `cd_thing.png` fill plus crisp top and bottom edges; at the last line, the preceding line fades away and the song-details footer remains fully readable beneath it. Also verify automatic following, manual wheel scrolling, seekable line clicks, wait dots during long gaps, and readable plain lyrics. Switch tracks while it is open and confirm the loading state replaces the old lines; if the new track has no lyrics, the pane retracts cleanly.
...
- Confirm the Album tab promotes the album name into the main header and the current track into the subtitle, then shows only title, artist, first release date, genres, and its track list while keeping the current lifted cover unchanged; opening the tab must not trigger a replacement-artwork fetch or flash.
...
- Enter regular lyrics and confirm the prepared surface appears immediately without a blank program frame. In regular lyrics, the information-side pane, and fullscreen lyrics, confirm the active synchronized line uses one stretched `cd_thing.png` fill with crisp top and bottom edges. Check an active line that wraps to three or more rows: its container must stay fully below the regular lyrics header instead of clipping at the viewport top. Seek between lines, switch tracks, and reopen fullscreen lyrics at the same playback position; the glow must follow the active row every time, while inactive and plain lines stay unframed.
...
- In regular lyrics, let several synchronized lines advance without scrolling and confirm every completed line disappears instead of peeking through above the active row. Scroll upward and confirm those previous lines return immediately for browsing.
